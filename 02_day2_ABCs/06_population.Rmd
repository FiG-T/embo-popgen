...
\end{equation} where $\frac{1}{B(\alpha,\beta)}$ is simply a normalization term which does not depend on $p$.
...
Then calculate the maximum a posteriori value, 95% credible intervals, and notable quantiles.
...
posterior.prob <- dbeta( 
  x = seq(0,1,0.01),
  shape1 = (k+1), 
  shape2 = (n-k+1)
)

plot(posterior.prob, type = "l")

max(posterior.prob)
quantile(posterior.prob)
MAP <- ((k+1) - 1)/((k+1) + (n-k+1) -2)

credLB <- qbeta(0.025,(k+1), (n-k+1))
credUB <- qbeta(0.975,(k+1), (n-k+1))
```
