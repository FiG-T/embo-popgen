...
folder.fastSimcoal2 <- "/home/grover/";

setwd(folder.fastSimcoal2);

model.admixed.pop <- function(
      sample_1, 
      sample_2, 
      sample_admix, 
      ...
      time_admixture, 
      percentage_from_population_1, 
      time_split, 
      effective_population_size_ancestral, 
      number_of_blocks
      ){
  lines <- c(
  ...
  data.t <- read.table(
    file=paste(folder.fastSimcoal2,"DemographicModelSplitR/DemographicModelSplitR_1_1.gen", sep=""),
    header = T
    );
...
# run simulation
sim <- model.admixed.pop(
  sample_1, sample_2,
  sample_admix,
  effective_population_size_1, effective_population_size_2, effective_population_size_admixed, 
  time_admixture, 
  percentage_from_population_1, 
  time_split, 
  effective_population_size_ancestral, number_of_blocks
  )
...
plot(
  mds.result$points[,1:2], 
  xlab = paste("DIM 1 (",round(100*mds.result$eig[1]/sum(mds.result$eig),2) ," %)"),
  ylab = paste("DIM 2 (",round(100*mds.result$eig[2]/sum(mds.result$eig),2) ," %)")
  )
points(
  mds.result$points[pop=="A",1], 
  mds.result$points[pop=="A",2],
  pch = 19, col ="red"
  );
points(
  mds.result$points[pop=="B",1], 
  mds.result$points[pop=="B",2], 
  pch = 19, col ="blue"
  );
...
extract.admixture <- function(points.mds)
{
  ...
  meanA <- mean(points.mds[popA,1]); # caluclate the mean for DIM1 for popA 
  meanB <- mean(points.mds[popB,1]); # caluclate the mean for DIM1 for popH 
  return(1-(points.mds[popAd,1]-meanA)/(meanB-meanA)); # calculate mean DIM1 for admixed and scale between 0 and 1 
}
...
for(rep in 1:100)
{
  # sample a percentage between 0 and 1
  percentage_from_population_1 <- runif(1,0,1);
  
  percentage.admixture[rep] <- percentage_from_population_1;
  
  sim <- model.admixed.pop(
    sample_1, sample_2, sample_admix,
    effective_population_size_1, effective_population_size_2, effective_population_size_admixed, 
    time_admixture, percentage_from_population_1, time_split, 
    effective_population_size_ancestral, number_of_blocks
    )
...
time.admixture <- rep(NA,50);
sd.percentage.admixture.estimated <- rep(NA,50);
for(rep in 1:50)
{
  # randomly sample generation time between 1 and 5000 generations ago. 
...
read.table(
  file=paste(project@runs[best][[1]]@directory,project@runs[best][[1]]@G.output.file, sep=""),
  header=F
  )
