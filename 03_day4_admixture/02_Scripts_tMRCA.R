...
model.single.pop.with.recent.population.change.tMRCA <- function(
  ...
  data.t <- read.table(file=paste(
    folder.fastSimcoal2,"/DemographicModelSplitR/DemographicModelSplitR_mrca.txt", sep=""), 
    header = T,
    skip = 4
  );
  return(data.t);
...
data <- model.single.pop.with.recent.population.change.tMRCA(
  population_size_in_present= 10000, # number of chromosomes 
  effective_population_size_1 = 1000, # number of chromosome
  time_change_population_size = 500, 
  number_of_blocks = 1000 # number of loci 
  )
...
plot(x = time.change, y = mean.log.tmrca, type = "l")
...
data <- model.single.pop.with.recent.population.change.tMRCA(2000, 500, 100, 1000)
...
bottleneck.data <- model.single.pop.with.bottleneck.tMRCA(
